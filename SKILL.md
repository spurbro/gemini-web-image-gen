...
1. **纯粹透明透传（与业务提示词解耦）**：
   - 本技能**不包含任何特定业务的提示词工程**。不同项目、不同需求的提示词由调用方 Agent 自行决定，本技能仅作为高可靠的“传输管道”，对提示词做 100% 原样透明透传。
2. **双重门禁质检机制 (Dual-Gated Hard Assertion)**：
   - **Gate 1（发送前附件门禁）**：触发 CDK 上传菜单后，硬性检测输入框内部是否切实渲染出图片附件缩略卡片（Attachment Chip）。若未检测到附件，立即阻断抛错，**绝对杜绝因上传延迟导致的“纯文本盲发降级”**；
   - **Gate 2（发送后消息门禁）**：消息发出后审计 DOM 用户消息气泡，确保服务端切实接收到了图片。
3. **单一会话持久管理 (Persistent Single Session)**：
   - 支持传入已有会话链接（`--url <chat_url>`），在同一对话中保持多轮生图上下文与连续性，避免滥开新标签页。
4. **内存原生 Canvas Base64 无损提取**：
   - 绕过 Chrome 对临时 Blob URL 的跨域安全限制与防盗链机制，直接利用 `HTML5 Canvas.toDataURL('image/png')` 从 DOM 提取全分辨率原始 PNG 数据流落盘。
5. **零环境耦合与自动自愈**：
   - 无任何项目写死路径，自动探针端口 `9222`，掉线时自动以独立 Profile（`~/.chrome-gemini-bridge`）拉起 Chrome，一次登录永久保持登录态。
...
graph TD
    ...
    I -->|确认送达| J["轮询 DOM 中新渲染的 Imagen 3 大图元素"]
...
| `--ref` | `-r` | 图生图参考图片路径（触发图生图及双重门禁） | 选填 | `null` |
